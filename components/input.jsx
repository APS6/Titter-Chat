...
export default function Input() {
  ...
  const dimensionsHandler = (img, key) => {
    const imageWidth = img.naturalWidth.toString()
    const imageHeight = img.naturalHeight.toString()

    const newImages = [...images]
    const imgIndex = newImages.findIndex((img) => img.key === key)
    newImages[imgIndex] = { ...newImages[imgIndex], width: imageWidth, height: imageHeight}
            ...
            return (
              <div className="relative bg-[#343434] rounded" key={image.key}>
                <Image
                  ...
                  onLoadingComplete={(img) => dimensionsHandler(img, image.key)}
                />
                <div
